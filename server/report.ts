...
export async function handleReport(body: Form, caller: string): Promise<Reply> {
  ...
  if (stage === "route") {
    ...
    const ips = [...new Set((net?.routes || []).flatMap((r) => r.hops.map((h) => h.ip)))]
      .filter((ip) => !/^(10\.|127\.|192\.168\.|172\.(1[6-9]|2\d|3[01])\.|100\.(6[4-9]|[7-9]\d|1[01]\d|12[0-7])\.)/.test(ip)).slice(0, 80)
  ...
  if (stage === "net") {
    ...
    if (asJson) return json({ ok: true, stage, network: net && { ...net, routes: net.routes.map((r) => ({ ...r, line: line(r) })) }, bgp })
